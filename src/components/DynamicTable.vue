  ...
  <div class="dynamic-table">
    ...
    <transition name="el-zoom-in-bottom">
      <div v-show="selectionRows.length" class="tool-wrap mb10">
        <span class="count ml10 mr10">已选中 {{ selectionRows.length || 0 }} 项{{ selectionRows.customizedMsg }}</span>
        ...
        <el-button
          ...
          >{{ item.name }}</el-button>
        <span class="close-btn fr mr10" @click.stop="toolCloseHandler">
    ...
    <el-table
      ...
      :key="dynamicTableKey"
      :data="rows"
      :row-key="(row, index) => { return index; }"
        ...
        <template slot-scope="{ row }">
          <div>
            <el-link
              type="primary"
              :underline="false"
              @click.stop="actionHandler(row, validActions(row, itemActionConfig)[0])"
              >{{ validActions(row, itemActionConfig)[0].name }}</el-link>
            <el-divider v-if="validActions(row, itemActionConfig).length > 1" direction="vertical"></el-divider>
            <el-dropdown
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  v-for="action in validActions(row, itemActionConfig).slice(1)"
                  :key="action.type"
                  ...
                  <el-upload
                    ...
                    :on-change="(file, fileList) => { importHandler(file, fileList, row); }"
                    :before-upload="(file) => { return beforeUpload(file, action.upload(row).fileType, action.upload(row).fileSize); }"
...
import TableFilter from './filterTag.vue';
import FilterTag from './tableFilter.vue';
...
export default {
  ...
  methods: {
    ...
    validActions(row, itemActions) {
      ...
      actions = actions.filter((action) => !action.disabled).filter(action => action.isShow == null || action.isShow(row) == true);
    ...
    forceRender() {
      ...
      this.$nextTick(() => {
        ...
        this.rows.forEach(row => row.isSelected = (selectedIds.includes(row.id)));
        this.rows.filter((row) => row.isSelected === true).forEach((row) => this.$refs.table.toggleRowSelection(row, true));
        this.$refs.tableFilter.forEach(filter => filter.copyFrom(filter.item));
      });
    },
    getSummaries(params) {
      let sums = [];
      ...
      sums = columns.map((col, index) => {
        ...
        return this.summaryRecord ? this.summaryRecord[col.property] : '';
      });
      return sums;
    },
    // 行拖拽
    ...
    selectionChange(val) {
      ...
      this.selectionRows = this._selectionRows.reduce((r, row) => r.concat(row), []);
    ...
    filterHandler(filter) {
      ...
      this.filterTags = this.$refs.tableFilter.filter((filter) => filter.keyword).map((filter) => {
        return {
...
//      this.toFilters();
    ...
    removeTagHandler(filterTag) {
      ...
      let tableFilter=this.$refs.tableFilter.find(_filter=>_filter.item.field==filterTag.field)
      if(tableFilter) tableFilter.reset();
      this.filterTags = this.filterTags.length && this.filterTags.filter((item) => item.field !== filterTag.field);
    ...
    removeSortTagHandler(tag) {
      ...
      this.sortTags = this.sortTags.length && this.sortTags.filter((item) => item.field !== tag.field);
    ...
    toQuery() {
      ...
      this.$refs.tableFilter.forEach(filter => {
        let column = this.columns.find(column => column.field == filter.field && column.isShow == true);
        let value = filter.keyword;
        if (column.fieldType == 'String') {
          if (column.filter.filterType == 'select') {
            value = value instanceof Array && value.length ? value.map(element => element.toString()) : [];
          } else {
        ...
        if (column.fieldType == 'Boolean') {
          if (column.filter.filterType == 'select') {
            value = value instanceof Array && value.length ? value.map(element => Boolean(element)) : [];
          } else {
        ...
        if (column.fieldType == 'BigDecimal' || column.fieldType == 'Long') {
          if (column.filter.filterType == 'select') {
            value = value instanceof Array && value.length ? value.map(element => Number(element)) : [];
          } else {
        ...
        if (column.filter.opr == 'between' && (filter.keywordFrom || filter.keywordTo)) value = [Number(filter.keywordFrom) || 0, Number(filter.keywordTo) || 0];
        if (column.filter.opr == 'between' && !(value instanceof Array)) value = null;
        q[column.filter.opr](column.field, value);
      });
      let result = q.toArray().map(query => { return query.value instanceof Array ? (query.value.length > 0 ? query : null) : query }).filter(query => query && (query.value || query.value === 0 || query.value === false));
      return result;
    },
    toOrderBy() {
      return this.$refs.tableFilter.filter((filter) => filter.orderOpr).map((filter) => {
        return {
  ...
  //解决过滤/排序序列化和反序列号
  toFilters(){
    let filters=this.$refs.tableFilter.filter(filter=> ((filter.keyword!='')||(filter.keywordFrom!='')||(filter.orderOpr!=""))).map(filter=>filter.copyTo({}))
//    this.fromFilters(filters)
   ...
   fromFilters(configs,callback,isRefresh=true){
     ...
     this.filterTags=this.$refs.tableFilter.filter(filter => filter.keyword).map(filter=>filter.toFilterTag());
     this.sortTags=this.$refs.tableFilter.filter(filter => filter.orderOpr).map(filter=>filter.toSortTag());
